...
class Evaluator:
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        with open_npz_array(npz_path, "arr_0") as reader:
            return self.compute_activations(
                reader.read_batches(self.batch_size))

    def compute_activations(
            self,
            batches: Iterable[np.ndarray]) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def compute_inception_score(self,
                                ...
                                split_size: int = 5000) -> float:
        softmax_out = []
        for i in range(0, len(activations), self.softmax_batch_size):
            acts = activations[i:i + self.softmax_batch_size]
            softmax_out.append(
                self.sess.run(self.softmax,
                              feed_dict={self.softmax_input: acts}))
    ...
    def compute_prec_recall(
            self, activations_ref: np.ndarray,
            activations_sample: np.ndarray) -> Tuple[float, float]:
        radii_1 = self.manifold_estimator.manifold_radii(activations_ref)
        radii_2 = self.manifold_estimator.manifold_radii(activations_sample)
        pr = self.manifold_estimator.evaluate_pr(activations_ref, radii_1,
                                                 activations_sample, radii_2)
...
class ManifoldEstimator:
    ...
    def manifold_radii(self, features: np.ndarray) -> np.ndarray:
        ...
        distance_batch = np.zeros([self.row_batch_size, num_images],
                                  dtype=np.float32)
        ...
        for begin1 in range(0, num_images, self.row_batch_size):
            ...
            for begin2 in range(0, num_images, self.col_batch_size):
                ...
                distance_batch[
                    0:end1 - begin1,
                    begin2:end2] = self.distance_block.pairwise_distances(
                        row_batch, col_batch)

            # Find the k-nearest neighbor from the current batch.
            radii[begin1:end1, :] = np.concatenate(
                [
                    x[:, self.nhood_sizes] for x in _numpy_partition(
        ...
        if self.clamp_to_percentile is not None:
            max_distances = np.percentile(radii,
                                          self.clamp_to_percentile,
                                          axis=0)
            radii[radii > max_distances] = 0
        return radii

    def evaluate(self, features: np.ndarray, radii: np.ndarray,
        ...
        for begin1 in range(0, num_eval_images, self.row_batch_size):
            ...
            max_realism_score[begin1:end1] = np.max(
                radii[:, 0] / (distance_batch[0:end1 - begin1, :] + self.eps),
                axis=1)
    ...
    def evaluate_pr(
        ...
        """
        features_1_status = np.zeros([len(features_1), radii_2.shape[1]],
                                     dtype=np.bool)
        features_2_status = np.zeros([len(features_2), radii_1.shape[1]],
                                     dtype=np.bool)
        for begin_1 in range(0, len(features_1), self.row_batch_size):
            ...
            for begin_2 in range(0, len(features_2), self.col_batch_size):
                ...
                batch_1_in, batch_2_in = self.distance_block.less_thans(
                    batch_1, radii_1[begin_1:end_1], batch_2,
...
class DistanceBlock:
    ...
    def __init__(self, session):
        ...
        with session.graph.as_default():
            self._features_batch1 = tf.placeholder(tf.float32,
                                                   shape=[None, None])
            self._features_batch2 = tf.placeholder(tf.float32,
                                                   shape=[None, None])
            ...
            self.distance_block = tf.cond(
                ...
                lambda: _batch_pairwise_distances(self._features_batch1, self.
                                                  _features_batch2),
            )
            ...
            self._batch_1_in = tf.math.reduce_any(dist32 <= self._radii2,
                                                  axis=1)
            self._batch_2_in = tf.math.reduce_any(dist32 <= self._radii1[:,
                                                                         None],
                                                  axis=0)
...
class StreamingNpzArrayReader(NpzArrayReader):
    ...
    def read_batch(self, batch_size: int) -> Optional[np.ndarray]:
        ...
        return np.frombuffer(data,
                             dtype=self.dtype).reshape([bs, *self.shape[1:]])
...
def _numpy_partition(arr, kth, **kwargs):
    ...
    with ThreadPool(num_workers) as pool:
        return list(pool.map(partial(np.partition, kth=kth, **kwargs),
                             batches))
