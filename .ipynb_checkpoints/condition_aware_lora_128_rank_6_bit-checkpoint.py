...
################# Quantization rate #################
if bit_depth == "6":
    print("Quantize to 6 bit")
    quantize_to_6bit_and_back(model)
elif bit_depth == "7":
    print("Quantize to 7 bit")
...
ckpt_path = f"/n/holylabs/LABS/wattenberg_lab/Users/yidachen/weights_backup/dit_{bit_depth}bit_{rank}r.pth"
...
for epoch in tqdm(range(start_epoch, epochs)):
    ...
    save_image(samples, f"temp_result/{rank}_r_{bit_depth}_bit/sample_{epoch}_{rank}_r_{bit_depth}_bit.png", nrow=int(samples_per_row),
               normalize=True, value_range=(-1, 1))
    samples = Image.open(f"temp_result/{rank}_r_{bit_depth}_bit/sample_{epoch}_{rank}_r_{bit_depth}_bit.png")
    display(samples)
    ...
    torch.save({
            ...
            'loss': all_loss,
            }, f"/n/holylabs/LABS/wattenberg_lab/Users/yidachen/weights_backup/dit_{bit_depth}bit_{rank}r.pth")
